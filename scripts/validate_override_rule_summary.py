...
from override_common import OVERRIDE_RULE_SUMMARY_COLUMNS as OUTPUT_COLUMNS
...
def run_validations(df: pd.DataFrame) -> list[str]:
    ...
    errors.extend(
        validate_required_text(
            df,
            ["rule_id", "question_id", "match_type", "pattern", "override_category_id", "override_category_name", "needs_human_review", "priority"],
        )
    )
    errors.extend(validate_boolean_column(df, "needs_human_review"))
    errors.extend(validate_integer_columns(df, ["priority", "hit_count", "unique_answer_count"]))
    errors.extend(validate_no_duplicate_rule_ids(df))
    return errors


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate 05_classification/override_rule_summary.csv")
    parser.add_argument("--input", required=True, type=Path, help="Path to override_rule_summary.csv")
...
def main() -> None:
    ...
    log_payload = {
        "event": "validate_override_rule_summary",
