...
`init-question` は初期化済み project に対してだけ実行する前提とする。
...
2. `python scripts/pipeline.py normalize ...` で済むならその公開I/Fを使う
...
- `02_screening` の補助成果物として `duplicate_responses.csv` を出してよい
- 初期実装では `question_id` + 正規化後 `answer_text` の完全一致だけを重複候補とする
- 重複候補は分類から除外せず、監査用に記録する

共通 `duplicate-check` CLI を使う場合は、group 内件数、canonical 行、正規化後 `answer_text` の一致を出力前に自己検査し、不整合な `duplicate_responses.csv` を書き出さない。
...
- `projects/{project_name}/02_screening/screened_responses.csv`
- `projects/{project_name}/questions/{question_id}/03_embeddings/embedding_requests.csv`
...
- `projects/{project_name}/questions/{question_id}/04_clustering/cluster_summary.csv`
- `projects/{project_name}/questions/{question_id}/05_classification/final_labels.csv`
- `projects/{project_name}/questions/{question_id}/06_review/review_log.csv`
...
共通 `embeddings` CLI を使う場合は、入力 `screened_responses.csv`、`embedding_requests.csv`、`embedding_metadata.json`、必要に応じて `embeddings.npy` / `embedding_failures.csv` の整合を出力前に自己検査し、`prepared` / `completed` / `failed` の状態に合わない成果物を書き出さない。
...
共通 `clustering` CLI を使う場合は、入力 `embedding_requests.csv` / `embeddings.npy` と、出力 `clusters.csv`、`cluster_summary.csv`、`clustering_metadata.json` の整合を出力前に自己検査し、不整合な成果物を書き出さない。
...
共通 `classification` CLI を使う場合は、入力 `screened_responses.csv`、`category_master.csv`、必要に応じて `manual_override_rules.csv` と、出力 `final_labels.csv` の整合を出力前に自己検査し、不整合な成果物を書き出さない。
...
共通の override 候補系 CLI を使う場合は、`review_corrections.csv`、`manual_override_candidates.csv`、`manual_override_rules.csv`、`override_rule_hits.csv`、`override_rule_summary.csv` の整合を出力前に自己検査し、不整合な成果物を書き出さない。
...
- `projects/{project_name}/questions/{question_id}/06_review/review.log`
...
```bash
python scripts/pipeline.py validate-log \
  --project-name {project_name} \
  --log-name pipeline.log
```
