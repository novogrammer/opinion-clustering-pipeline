...
案件ごとの元データは、そのまま本パイプライン入力に合うとは限らない。  
...
5. 以後の Embedding、クラスタリング、分類は変換後データを基準に進める

Codex に依頼する内容の例:

- Excel や CSV から必要列だけを抽出する
- 列名を `response_id`, `question_id`, `question_text`, `question_intent`, `answer_text` に合わせる
- 複数シートや複数ファイルを結合する
...
- `projects/{project_name}/01_processed/embedding_inputs.csv`
- `projects/{project_name}/99_logs/raw_to_processed_mapping.md`
- `projects/{project_name}/02_embeddings/embeddings_{question_id}.npy`
- `projects/{project_name}/02_embeddings/embedding_metadata_{question_id}.json`
- `projects/{project_name}/03_clustering/clusters_{question_id}.csv`
- `projects/{project_name}/03_clustering/cluster_summary_{question_id}.csv`
- `projects/{project_name}/04_classification/final_labels_{question_id}.csv`
- `projects/{project_name}/05_review/review_log.csv`
