...
from review_common import REVIEW_CORRECTION_COLUMNS as OUTPUT_COLUMNS
...
def validate_correction_type(df: pd.DataFrame) -> list[str]:
    ...
    invalid_rows = [
        str(index + 1)
        for index, value in enumerate(df["correction_type"].astype(str).str.lower())
        if value not in allowed
    ]
    if not invalid_rows:
        return []
    return [f"Invalid correction_type values at rows: {', '.join(invalid_rows)}"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate 06_review/review_corrections.csv")
...
def main() -> None:
    ...
    log_payload = {
        "event": "validate_review_corrections",
