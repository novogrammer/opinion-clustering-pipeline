...
def validate_match_type(df: pd.DataFrame) -> list[str]:
...
def run_validations(df: pd.DataFrame) -> list[str]:
    errors: list[str] = []
    errors.extend(validate_required_text(df, ["rule_id", "question_id", "match_type", "pattern", "override_category_id", "override_category_name", "needs_human_review", "priority"]))
    errors.extend(validate_no_duplicate_rule_ids(df))
    errors.extend(validate_match_type(df))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate 05_classification/manual_override_rules.csv")
    parser.add_argument("--input", required=True, type=Path, help="Path to manual_override_rules.csv")
...
def main() -> None:
    ...
    validate_required_columns(df, OVERRIDE_RULE_COLUMNS)
    errors = run_validations(df)

    log_payload = {
        "event": "validate_override_rules",
