...
1. 元データを `projects/{project_name}/data/raw/` に置く
2. Codex に、元データの列構造と不要列の扱いを確認させる
3. Codex に、パイプライン必須カラムへ変換する処理を作らせる
4. 変換後データを `projects/{project_name}/data/processed/` に保存する
...
- 元データは `raw/` に保持し、直接上書きしない
- Codex が行った変形は、再実行できる形で残す
- 変形仕様は案件ごとに `config/` または `logs/` に記録する
...
- `projects/{project_name}/logs/raw_to_processed_mapping.md`
...
- `projects/{project_name}/logs/raw_to_processed.log`
