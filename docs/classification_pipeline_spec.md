...
`category_master.csv` と `category_master_draft.csv` の想定カラム:
...
- LLM は `category_master_draft.csv` の初版を作る
- 同じ内容を初版 `category_master.csv` としても保存する
- 人は `category_master.csv` を編集して確定版にする
- `category_master_draft.csv` は自動生成の初版を残す補助成果物とする
