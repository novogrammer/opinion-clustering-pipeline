...
共通 `classification` CLI を使う場合は、入力 `screened_responses.csv` と `category_master.csv`、出力 `final_labels.csv` の整合を出力前に自己検査し、不整合な成果物を書き出さない。
...
- `confidence` が閾値未満
- `その他` に分類された回答
- `needs_human_review = true` の回答

確認後は、修正結果を再学習用データではなく評価ログとして保持する。

標準フローでは `06_review` の主成果物を `review_log.csv` とし、追加の review 集計は任意作業として扱う。

`review_log.csv` は `final_labels.csv` をもとに機械的に作る。  
標準フローの review 判定は最小限とし、追加の意味解釈は行わない。

初期運用で使う `review_trigger`:
...
初期運用で使う `review_priority`:

- `needs_human_review` / `low_confidence` / `fallback_category` を含む場合は `high`
- それ以外は `low`

初期運用で使う `review_status`:

- `typical_match` のみなら `skipped`
- それ以外は `pending`
